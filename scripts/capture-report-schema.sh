...
output=${1:-"$repo_root/docs/current/report-v2-fields.txt"}
tmp=$(mktemp)
metadata_tmp=$(mktemp)
filtered_metadata_tmp=$(mktemp)
compiler_artifacts_tmp=$(mktemp)
audit_inventory_tmp=$(mktemp)
fixture_inventory_tmp=$(mktemp)
capture_target=$(mktemp -d)
cargo_workdir=$(mktemp -d)
isolated_cargo_home=
source_snapshot=$(mktemp -d)
trap 'rm -f "$tmp" "$metadata_tmp" "$filtered_metadata_tmp" "$compiler_artifacts_tmp" "$audit_inventory_tmp" "$fixture_inventory_tmp"; rm -rf "$capture_target" "$cargo_workdir" "$isolated_cargo_home" "$source_snapshot"' EXIT

cd "$repo_root"
capture_script_rel=scripts/capture-report-schema.sh
...
if [[ $rustc_release != "$required_rustc_release" ||
  ...
  printf 'report schema capture requires the pinned Phase 0 toolchain:\n' >&2
  printf '  rustc %s (%s)\n  cargo %s (%s)\n  host %s\n' \
    "$required_rustc_release" "$required_rustc_commit" \
    "$required_cargo_release" "$required_cargo_commit" "$required_host" >&2
  printf 'observed:\n%s\n%s\n' "$rustc_verbose" "$cargo_verbose" >&2
  exit 1
fi
default_output="$repo_root/docs/current/report-v2-fields.txt"
...
# Stage only cache entries named by the product or extractor lockfile. Phase 0 compatibility
...
' "$repo_root/Cargo.lock" \
  "$repo_root/tests/support/report-source-audit/Cargo.lock" | \
  LC_ALL=C sort -u > "$filtered_metadata_tmp"
...
# Cargo and the Serde fixture run only against this private copy. A concurrent
# checkout edit cannot alter the compiled bytes or the emitted fixture shapes.
...
perl -MJSON::PP -0777 -ne '
  ...
  while (@queue) {
    ...
    push @queue, map { $_->{"pkg"} } @{$node->{"deps"}};
  }
...
extractor_tree_sha256=$(hash_tree_paths \
...
run_cargo check --locked --manifest-path "$repo_root/Cargo.toml" \
  --example capture_report_schema --target "$host_triple" --target-dir "$capture_target" \
  --message-format=json-render-diagnostics > "$compiler_artifacts_tmp"
validate_compiler_inputs "$capture_target" "$compiler_artifacts_tmp" \
  examples/capture_report_schema.rs
run_cargo check --locked \
  --manifest-path "$repo_root/tests/support/report-source-audit/Cargo.toml" \
  --target "$host_triple" --target-dir "$capture_target" \
  --message-format=json-render-diagnostics >> "$compiler_artifacts_tmp"
validate_compiler_inputs "$capture_target" "$compiler_artifacts_tmp" \
  examples/capture_report_schema.rs \
  tests/support/report-source-audit/Cargo.toml \
  tests/support/report-source-audit/Cargo.lock \
  tests/support/report-source-audit/src/main.rs
run_cargo run --locked --quiet \
  --manifest-path "$repo_root/tests/support/report-source-audit/Cargo.toml" \
  --target "$host_triple" --target-dir "$capture_target" \
  -- "$repo_root/src/report.rs" > "$audit_inventory_tmp"
LC_ALL=C sort -u -o "$audit_inventory_tmp" "$audit_inventory_tmp"

{
  printf '%s\n' \
    'artifact-format report-v1-serde-fixture-shapes/v7' \
    'extractor-version serde-bounded-fixture-json/v6' \
    ...
    "dependency-build-scripts-sha256 $dependency_build_scripts_sha256" \
    "dependency-sources-sha256 $dependency_sources_sha256"
  run_cargo run --locked --quiet --manifest-path "$repo_root/Cargo.toml" \
    --example capture_report_schema --target "$host_triple" --target-dir "$capture_target"
} > "$tmp"
sed -n 's/^struct ccwrapped:://p' "$tmp" | LC_ALL=C sort -u > "$fixture_inventory_tmp"
if ! cmp -s "$audit_inventory_tmp" "$fixture_inventory_tmp"; then
  printf 'report source audit and fixture inventories disagree:\n' >&2
  diff -u "$audit_inventory_tmp" "$fixture_inventory_tmp" >&2 || true
  exit 1
fi
...
rm -f "$metadata_tmp" "$filtered_metadata_tmp" "$compiler_artifacts_tmp" \
  "$audit_inventory_tmp" "$fixture_inventory_tmp"
rm -rf "$capture_target"
rm -rf "$cargo_workdir" "$isolated_cargo_home" "$source_snapshot"
trap - EXIT
